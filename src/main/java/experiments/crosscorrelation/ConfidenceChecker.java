    ...
    Vector<TimeSeriesObject> conf = new Vector<TimeSeriesObject>();
        ...
        for ( int i = 0; i < max ; i++ ) {
            ...
            for ( NodePair p1 : ccr[i][0].paare ) {
                ...
                histSHUFF.addValue( p1.signLevel );
                histNORMAL.addValue( p2.signLevel );

                zaehler1.addData( p1.signLevel );
                zaehler2.addData( p2.signLevel );
            ...
            v.add( histSHUFF );
            int maxB = 8;
            createHistogramm(v, maxB*15, 0, maxB);
            zaehler1.calcWS();
            zaehler2.calcWS();
