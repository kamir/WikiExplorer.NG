 ...
 * 
 * Here we use the max CC value for each cc-function on its specific tau ...
 */
 ...
 * @author kamir
 */
public class DistributionCheckChartsDetailedV3 {
    ...
    public static void doWorkNow(String fnBase, String inputset, boolean storSignificanz) throws FileNotFoundException, IOException {
        ...
        for( int c = 0; c < 3; c++ ) {
            for (int j = 0; j < 5; j++) {
                ...
                reihenMWArray[c][j] = new TimeSeriesObject("l=" + keysR[j] + ":" + keysC[c]);
                reihenSIGMAArray[c][j] = new TimeSeriesObject("l1=" + keysR[j] + ":" + keysC[c]);
                reihenSIGMAArray2[c][j] = new TimeSeriesObject("l2=" + keysR[j] + ":" + keysC[c]);
                reihenSIGMAArray3[c][j] = new TimeSeriesObject("l3=" + keysR[j] + ":" + keysC[c]);
            } 
        }
                ...
                while (br.ready()) {
                    ...
                    if (mr != null) {
                        ...
                        if( rec.p1 > (1.0-alpha) && rec.p2 > (1.0-alpha) ) {
                            avCC[ (int) rec.tau + 5][z][r] = avCC[ (int) rec.tau + 5][z][r] + rec.cc[(int) rec.tau + 5];
                            counter[ (int) rec.tau + 5][z][r] = counter[ (int) rec.tau + 5][z][r] + 1.0; 
                            s[z][r]=s[z][r]+1.0;
                        }    
            ...
            for (int j = 0; j < 5; j++) {  // fuer jede Fensterlänge ...
                for (int i = 0; i < 11; i++) {  // fuer jedes tau zw.  -5 ... 0 ... 5
                    ...
                    if (counter[i][j][c] != 0.0) {
                        av[i][j][c] =  counter[i][j][c] / s[j][c] ; // avCC[i][j][c] /
                    }
